...
const progressConnections = new Map<
  string,
  Set<ServerWebSocket<ProgressSocketData>>
>();
...
async function startProgressMonitoring(clipboardId: string) {
  ...
  const interval = setInterval(async () => {
    try {
      ...
      const fileInfo = await filesDBClient
        .getFileInfo(clipboardId)
        .catch(() => null);
      ...
      const progressData = {
        ...
        chunks_uploaded_to_blockchain:
          status.chunks_uploaded_to_blockchain || 0,
...
const app = new Hono<{ Bindings: { server: Server } }>().basePath(
...
const FROM_EMAIL =
  process.env.FROM_EMAIL || "noreply@copypal.usecases.arkiv.network";
...
app.use(
  ...
  cors({
    origin: [
      "https://copypal.usecases.arkiv.network",
      "http://copypal.usecases.arkiv.network",
      "http://localhost:5173",
      "http://localhost:8881",
    ],
...
app.post("/v1/auth/verify", async (c) => {
  try {
    ...
    if (
      ...
      Date.now() - authSession.createdAt > fiveMinutes
    ) {
      return c.json(
        {
...
async function uploadFileWithChunking(
  ...
  console.log(
    `Chunking large file: ${fileName} (${totalSize} bytes) into ${totalChunks} chunks`,
  );
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    console.log(
      `Uploading chunk ${i + 1}/${totalChunks} (${chunkBuffer.length} bytes)`,
    );

    try {
      const chunkId = await filesDBClient.uploadFile(
        chunkBuffer,
        chunkFileName,
  ...
  const metadataId = await filesDBClient.uploadFile(
    metadataBuffer,
...
async function downloadFileFromChunks(
  metadataId: string,
): Promise<{ data: Buffer; info: any }> {
  // Download metadata
  const { data: metadataBuffer } = await filesDBClient.downloadFile(metadataId);
  const metadata = JSON.parse(metadataBuffer.toString());

  console.log(
    `Assembling chunked file: ${metadata.originalFileName} from ${metadata.totalChunks} chunks`,
  );
  ...
  for (let i = 0; i < metadata.totalChunks; i++) {
    ...
    console.log(
      `Downloading chunk ${i + 1}/${metadata.totalChunks}: ${chunkId}`,
    );
  ...
  if (combinedBuffer.length !== metadata.totalSize) {
    throw new Error(
      `File size mismatch: expected ${metadata.totalSize}, got ${combinedBuffer.length}`,
    );
  }

  console.log(
    `File assembly completed: ${metadata.originalFileName} (${combinedBuffer.length} bytes)`,
  );
  ...
  try {
    ...
    if (expiresAtStr) {
      ...
      effectiveTtlDays = ttlDays
        ? Math.min(ttlDays, calculatedTtl)
        : calculatedTtl;
    }
    ...
    let fileId: string;
    if (kind === "text") {
      fileId = await filesDBClient.uploadText(
        content,
        "clipboard.txt",
        effectiveTtlDays,
        owner,
      ...
      const CHUNK_SIZE = 512 * 1024; // 512KB
      console.log(
        `File size: ${buffer.length} bytes, chunk size threshold: ${CHUNK_SIZE}`,
      );
      if (buffer.length > CHUNK_SIZE) {
        console.log(`File needs chunking, starting chunking process...`);
        fileId = await uploadFileWithChunking(
          buffer,
          fileName || "file",
        ...
        fileId = await filesDBClient.uploadFile(
          buffer,
          fileName || "file",
          fileType || "application/octet-stream",
          effectiveTtlDays,
    ...
    console.log(
      `File uploaded to FileDB: ${fileId}, waiting for blockchain upload...`,
    );
    ...
    while (!uploadCompleted && Date.now() - startTime < maxWaitTime) {
      try {
        ...
        const chunks =
          progressInfo.chunks_uploaded ||
          status.chunks_uploaded_to_blockchain ||
          0;
        const totalChunks =
          progressInfo.total_chunks || status.total_chunks || 0;
        const percentage =
          progressInfo.percentage || status.progress_percentage || 0;
        ...
        if (progressInfo.last_chunk_uploaded_at) {
          ...
          const timeSinceLastUpload = Math.round(
            (Date.now() - lastUpload.getTime()) / 1000,
          );
        ...
        const message = getErrorMessage(error);
        console.log(
          `Waiting for blockchain upload... (${Math.round((Date.now() - startTime) / 1000)}s)`,
          message,
        );
        await new Promise((resolve) => setTimeout(resolve, 1000));
      }
    ...
    if (uploadCompleted && entities) {
      ...
      return c.json({
        ...
        blockchainInfo: {
          ...
          explorerUrls: {
            ...
            chunks: entities.chunk_entity_keys.map(
              (chunkKey: string) =>
      ...
      return c.json({
        ...
        status: "uploading",
        message:
          "File uploaded to FileDB, blockchain upload in progress. Entity keys will be available once upload completes.",
      });
    }
  } catch (error) {
    const message = getErrorMessage(error);
...
app.get("/v1/clipboard/:id/download", async (c) => {
  ...
  try {
    ...
    return c.json(
      {
        error: "Failed to download file from FilesDB: " + message,
        success: false,
      },
      500,
    );
...
app.post("/v2/clipboard/smart-upload", async (c) => {
  try {
    ...
    if (kind === "text") {
      const { fileId, quotaUsed } =
        await enhancedFilesDBClient.uploadTextWithQuotaCheck(
          content,
          "clipboard.txt",
          effectiveTtlDays,
          owner,
    ...
    console.log(
      `Enhanced upload completed: ${result.fileId}, method: ${result.method}`,
    );
...
app.get("/v2/session/:sessionKey/status", async (c) => {
  try {
    ...
    const status =
      await enhancedFilesDBClient.getUploadSessionStatus(sessionKey);
...
app.post("/v2/clipboard/resume-upload", async (c) => {
  try {
    ...
    if (!sessionKey || !fileData || !fileName) {
      throw new Error(
        "Missing required fields: sessionKey, fileData, fileName",
      );
    }
    ...
    message(ws: ServerWebSocket<ProgressSocketData>, message: string | Buffer) {
      try {
        ...
        if (data.type === "subscribe" && data.clipboard_id) {
          ...
          console.log(
            `WebSocket: Client subscribing to progress for ${clipboardId}`,
          );
