...
const WS_BASE =
  import.meta.env.MODE === "production"
    ? "wss://copypal.usecases.arkiv.network/ws"
    : "ws://localhost:19234/ws";

export function FileProgressBarWS({
  clipboardId,
  isUploading,
  onComplete,
}: FileProgressBarWSProps) {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === "progress") {
          ...
          setArkivProgress((prev) =>
            prev ? { ...prev, error: data.error } : null,
          );
        }
      } catch (error) {
    ...
    ws.onclose = (event) => {
      console.log(
        `WebSocket closed for ${clipboardId}`,
        event.code,
        event.reason,
      );
          ...
          <div className="flex justify-between text-xs">
            <span className="text-[#9AA7BD]">
              ...
              {connectionStatus === "connected" && (
                <span className="text-[#20C15A]"> ●</span>
              )}
              {connectionStatus === "connecting" && (
                <span className="text-yellow-500"> ●</span>
              )}
              {connectionStatus === "error" && (
                <span className="text-[#E85B5B]"> ●</span>
              )}
            </span>
            <span
              className={`${
